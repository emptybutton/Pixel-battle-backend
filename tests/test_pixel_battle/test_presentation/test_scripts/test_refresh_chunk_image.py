...
@fixture
def script(ok_file: StringIO, error_file: StringIO) -> Script:
...
async def test_ok(script: Script, ok_file: StringIO) -> None:
    argv[:] = ["_", "0", "0"]

    await script()

    output = ok_file.getvalue().split("\n")
...
async def test_with_invalid_args(
    script: Script, args: list[str], error_file: StringIO
) -> None:
    argv[:] = ["_", *args]

    await script()

    output = error_file.getvalue()
    assert output == "Coordinates must be numbers in range(0, 10).\n"


async def test_help_by_flag(script: Script, ok_file: StringIO) -> None:
    argv[:] = ["_", "--help"]

    await script()

    output = ok_file.getvalue()
    excepted_output = (
...
async def test_help_by_args(script: Script, ok_file: StringIO) -> None:
    argv[:] = ["_"]
