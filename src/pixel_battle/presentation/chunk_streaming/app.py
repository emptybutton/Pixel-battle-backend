...
from pixel_battle.presentation.chunk_streaming.routes import router
...
def app_with(container: AsyncContainer) -> FastAPI:
    ...
    description = (
        "ChunkStreaming is one of the APIs of the Pixel battle game, presented"
        " as a separate service through which clients can listen to changes"
        " occurring on a specific chunk."
        " \n\nPixel battle has open source code, which can be"
        f" found [here]({repo_url}) (including the code for this service)."
    )
    app = FastAPI(
        title="ChunkStreaming",
        version="0.1.0",
        ...
        openapi_url="/chunk-streaming/openapi.json",
        docs_url="/chunk-streaming/docs",
        redoc_url="/chunk-streaming/redoc",
        lifespan=lifespan,
    )
